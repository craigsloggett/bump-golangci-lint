...
VERSION=$(curl -sf https://proxy.golang.org/github.com/golangci/golangci-lint/v2/@latest | jq -r '.Version')

if [ -z "${VERSION}" ] || [ "${VERSION}" = "null" ]; then
  printf 'Failed to determine the latest golangci-lint version.\n' >&2
  exit 1
fi
